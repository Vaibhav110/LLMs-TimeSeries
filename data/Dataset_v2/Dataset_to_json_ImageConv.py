        ...
        for row in csv_list:
            ...
            if four_dates_count == (Data_Size+1):
                ...
                data_item = {
                    ...
                    "query_scalogram": f" Value on Nitrogen Dioxide in Los Angeles on ten consecutive days from {Combined_NO2_days[0]} to {Combined_NO2_days[Data_Size-1]}  during the {Data_season} {Data_year} season is {", ".join(str(s) for s in Combined_NO2_data)}. Analyze the provided {img_type_scalogram} of Nitrogen Dioxide for ten days and estimate the expected Nitrogen Dioxide value for the subsequent day that is {Data_day}.",
                    "image_path_mtf": f"{img_path[1]}",
                    "query_mtf": f" Value on Nitrogen Dioxide in Los Angeles on ten consecutive days from {Combined_NO2_days[0]} to {Combined_NO2_days[Data_Size-1]}  during the {Data_season} {Data_year} season is {", ".join(str(s) for s in Combined_NO2_data)}. Analyze the provided {img_type_mtf} of Nitrogen Dioxide for ten days and estimate the expected Nitrogen Dioxide value for the subsequent day that is {Data_day}.",
                    "image_path_spectrogram": f"{img_path[2]}",
                    "query_spectrogram": f" Value on Nitrogen Dioxide in Los Angeles on ten consecutive days from {Combined_NO2_days[0]} to {Combined_NO2_days[Data_Size-1]} during the {Data_season} {Data_year} season is {", ".join(str(s) for s in Combined_NO2_data)}.Analyze the provided {img_type_spectrogram} of Nitrogen Dioxide for ten days and estimate the expected Nitrogen Dioxide value for the subsequent day that is {Data_day}.",                    
                    "answers": f"{Data_NO2}"
                }
...
if IS_SCALOGRAM:
    ...
    img_path_prefix.append("Dataset_LA_10/Images_Scalogram/Data_")
    img_type_scalogram = 'Scalogram'
if IS_MTF:
    # img_path_prefix = "Dataset/Images_MTF/Data_"
    img_path_prefix.append("Dataset_LA_10/Images_MTF/Data_")
    img_type_mtf = 'Markov Transition Field'
if IS_SPECTROGRAM:
    # img_path_prefix = "Dataset/Images_Spectrogram/Data_"
    img_path_prefix.append("Dataset_LA_10/Images_Spectrogram/Data_")
...
json_file_path_train = r"C:\Users\vaibh\OneDrive\Documents\UCLA_Courses\M202A - Embedded Systems\LLMs_ReadingaMaterial\Dataset_v2\Output_dataset_image_mContext.json"
